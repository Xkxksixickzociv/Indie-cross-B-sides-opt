...
function onCreate()
	...
	for i = 0, getProperty('unspawnNotes.length')-1 do
		...
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'Darkness Ink Note' then
			setPropertyFromGroup('unspawnNotes', i, 'texture', 'bendy/NOTE_nmrunink');
...
function goodNoteHit(id, noteData, noteType, isSustainNote)
	if noteType == 'Darkness Ink Note' and songName ~= 'Last-Reel' and songName ~= 'Despair'then
		playSound('inked')
		Inkage = Inkage + 1
		...
		if Inkage == 1 then
			makeLuaSprite('SplashScreen','Damage01',0,0)
		elseif Inkage == 2 then
			makeLuaSprite('SplashScreen','Damage02',0,0)
		elseif Inkage == 3 then
			makeLuaSprite('SplashScreen','Damage03',0,0)
		elseif Inkage == 4 then
			makeLuaSprite('SplashScreen','Damage04',0,0)
